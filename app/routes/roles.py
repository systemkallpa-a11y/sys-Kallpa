...
@login_required
def roles():
    """Página de gestión de roles y accesos de usuarios"""
    # Validar que el usuario tenga acceso a Usuarios y Accesos
    num_documento = session.get('user_documento')
    
    # ID 4 = Configuración, ID 8 = Rol y Accesos (submenú)
    # Permitir si tiene: acceso completo A Configuración O acceso específico a Rol y Accesos
    ...
    if not (tiene_acceso_completo or tiene_acceso_roles):
        flash('No tienes acceso a Gestión de Accesos', 'danger')
...
def obtener_usuarios_accesos():
    """Obtener lista de usuarios con sus accesos a menús y submenús"""
    ...
    try:
        ...
        print("[USUARIOS_ACCESOS] Iniciando obtención de usuarios y accesos")
        
        # Llamar SP que retorna usuarios con información de accesos
        cursor.execute('CALL sp_ObtenerUsuariosAccesos()')
...
def obtener_accesos_usuario(num_documento):
    """Obtener accesos detallados de un usuario específico"""
    connection = get_db_connection()
    if not connection:
        return jsonify({'success': False, 'error': 'Error de conexión'}), 500
    ...
    try:
        ...
        cursor.execute('CALL sp_ObtenerAccesosUsuario(%s)', (num_documento,))
...
def obtener_menus_submenus():
    ...
    try:
        ...
        print("[MENUS_SUBMENUS] Obteniendo estructura de menús")
        ...
        print(f"[MENUS_SUBMENUS] {len(menus)} menús, {len(submenus)} submenús obtenidos")
...
def guardar_accesos_usuario():
    ...
    try:
        ...
        try:
            ...
            # Solo los pasamos al SP tal como están
            accesos_con_nombres = []
            for idx, acceso in enumerate(accesos):
                ...
                # VERIFICAR QUÉ TIPO DE DATOS LLEGARON
                if 'menu_nombre' in acceso:
                    ...
                    print(f"  - ⚠️ DETECTADO: id_menu = {acceso.get('id_menu')} (¡LLEGÓ ID, no NOMBRE!)")
                    # Si llegó ID, significa que el frontend SIGUE enviando IDs
                    return jsonify({
                        'success': False, 
                        'error': 'El frontend está enviando IDs, no NOMBRES. Frontend.js no fue actualizado correctamente.'
                    }), 400
                else:
                    print(f"  - ❌ ERROR: No hay ni menu_nombre ni id_menu")
                    return jsonify({'success': False, 'error': 'Estructura de accesos incorrecta'}), 400
                
                menu_nombre = acceso.get('menu_nombre')
                submenu_nombre = acceso.get('submenu_nombre')
                
                # Validar que al menos el menú tenga nombre
                if not menu_nombre:
                    return jsonify({'success': False, 'error': 'Nombre de menú requerido'}), 400
            ...
            print(f"[GUARDAR_ACCESOS] Parámetros:")
            ...
            print(f"\n[GUARDAR_ACCESOS] ✅ SP retornó resultado:")
            ...
            print(f"\n{'='*80}")
            print(f"[GUARDAR_ACCESOS] ✓ Operación completada exitosamente")
            print(f"{'='*80}\n")
        ...
        print(f"\n[GUARDAR_ACCESOS] ✗✗✗ ERROR GENERAL: {e}")
        print(f"[GUARDAR_ACCESOS] Tipo de error: {type(e).__name__}")
        import traceback
        traceback.print_exc()
        print()
